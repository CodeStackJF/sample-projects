...
var omit = (objToPluck, keysToPluck) => {
	const normalizeKey = (key) => typeof key === "number" ? `${key}` : key;
	const keysToOmit = new Set(keysToPluck.map(normalizeKey));
	return Reflect.ownKeys(objToPluck).filter((key) => !keysToOmit.has(normalizeKey(key))).reduce((result, key) => ({
		...result,
		[key]: objToPluck[key]
	}), {});
};
...
//# sourceMappingURL=object-C7YmCGEm.js.map